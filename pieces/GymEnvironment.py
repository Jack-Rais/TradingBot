...
class TradingEnv(gym.Env):
    ...
    def observation_spec(self):
        ...
        return Dict({
            ...
            'simbolo': Box(shape=(1, 5), 
                           dtype=np.int32,
                           low=0,
                           high=self.observer.tokenizer.vocab_size - 1),
                                        
            'titolo': Box(shape=(self.news_limit, 100), 
                          dtype=np.int32,
                          low=0,
                          high=self.observer.tokenizer.vocab_size - 1),

            'paragrafi': Box(shape=(self.news_limit, 512), 
                             dtype=np.int32,
                             low=0,
                             high=self.observer.tokenizer.vocab_size - 1),

            'prezzi': Box(shape=(50, 7), 
                          dtype=np.float32,
                          low=-np.inf,
                          high=np.inf)
        })
    ...
    def action_spec(self):
        ...
        return Box(
            ...
            dtype = np.int32,
            low = 0,
            high = 2 if self.neutrality else 1
        )
    ...
    def step(self, action):
        ...
        if self.limit_steps:
            ...
            if self.limit_steps < self.steps:
                ...
                return self.get_observation(), np.array(premio, dtype=np.float32), np.array(1), {}
        ...
        return self.get_observation(), np.array(premio, dtype=np.float32), np.array(0), {}